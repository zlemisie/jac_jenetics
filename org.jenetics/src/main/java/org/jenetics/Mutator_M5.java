	...
	extends AbstractAlterer<DoubleGene, C>
{
	 ...
	 */
	public Mutator_M5(final double probability) {
		super(probability);
	}
	...
	private int mutate(final MSeq<Chromosome<DoubleGene>> c, final int i, final double p) {
		...
		final MSeq<DoubleGene> genes = chromosome.toSeq().copy();
...
//				.count();
	...
	private DoubleGene upgrade1(DoubleGene x, DoubleGene uy){
		...
		return DoubleGene.of(v,RealFunction3.getMinV(),RealFunction3.getMinV());
	}
