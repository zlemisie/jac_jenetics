...
import org.jenetics.example.Ackley;
	...
	extends AbstractAlterer<DoubleGene, C>
{
	 ...
	 */
	public Mutator_M6_A(final double probability) {
		super(probability);
	}
	...
	private int mutate(final MSeq<Chromosome<DoubleGene>> c, final int i, final double p) {
		...
		genesM.set(0, DoubleGene.of(Math.max(val - change, Ackley.getMinV()), Ackley.getMinV(), Ackley.getMaxV()));
		genesW.set(0, DoubleGene.of(Math.min(val+change, Ackley.getMaxV()), Ackley.getMinV(), Ackley.getMaxV()));
...
//				.count();
	...
	/*private DoubleGene upgrade1(DoubleGene x, DoubleGene uy){
		...
		if (Ackley.eval2(dx, vW) < Ackley.eval2(dx, v)){
		v = vW;
		}
		if (Ackley.eval2(dx, vM) < Ackley.eval2(dx, v)){
			v = vM;
		}
		//System.out.println("v2:"+v);

		return DoubleGene.of(v,Ackley.getMinV(),Ackley.getMaxV());
	}*/

	private double testD(double a){
	if(a < Ackley.getMinV()) return Ackley.getMinV();
	if(a > Ackley.getMaxV()) return Ackley.getMaxV();
	return a;
	}
