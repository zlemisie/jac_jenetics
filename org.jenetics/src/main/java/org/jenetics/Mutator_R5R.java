...
import org.jenetics.example.RealFunction5Rastrigin;
 ...
 * @version 3.0
 */
public class Mutator_R5R<
	G extends Gene<Boolean, BitGene>,
	C extends Comparable<? super C>
>
	extends AbstractAlterer<BitGene, C>
{
	 ...
	 */
	public Mutator_R5R(final double probability) {
		super(probability);
	}
	...
	public int alter(
		...
		indexes(RandomRegistry.getRandom(), population.size(), p).forEach(i -> {
			final Phenotype<BitGene, C> pt = population.get(i);

			final Genotype<BitGene> gt = pt.getGenotype();
			final Genotype<BitGene> mgt = mutate(gt, p, alterations);

			final Phenotype<BitGene, C> mpt = pt.newInstance(mgt, generation);
	...
	private Genotype<BitGene> mutate(
		final Genotype<BitGene> genotype,
		final double p,
		final IntRef alterations
	) {
		final MSeq<Chromosome<BitGene>> chromosomes = genotype.toSeq().copy();
	...
	/*private int mutate(final MSeq<Chromosome<BitGene>> c, final int i, final double p) {
		return 0;
	}*/

	private int mutate(final MSeq<Chromosome<BitGene>> c, final int i, final double p) {

		/*return (int)indexes(RandomRegistry.getRandom(), c.length(), p)
				.peek(i -> c.set(i, c.get(i).newInstance()))
				.count();*/

		//System.out.println("i: "+i);
		final Chromosome<BitGene> chromosome = c.get(i);
		MSeq<BitGene> genes = chromosome.toSeq().copy();

		//final MSeq<Chromosome<BitGene>> cN = c.copy();

		final MSeq<BitGene> genesN = chromosome.toSeq().copy();

		//System.out.println("l: "+chromosome.length());

		//double val = genes.get(0).booleanValue();

		Random generator = new Random();
		int ran = generator.nextInt(chromosome.length());

		genesN.set(ran, BitGene.of(!(genesN.get(ran).booleanValue())));

		c.set(i, chromosome.newInstance(genesN.toISeq()));
...
//		return mutations;*//*
	 ...
	 */
	protected int mutate(final MSeq<BitGene> genes, final double p) {
...
//				.count();
	...
	if(a < RealFunction5Rastrigin.getMinV()) return RealFunction5Rastrigin.getMinV();
	if(a > RealFunction5Rastrigin.getMaxV()) return RealFunction5Rastrigin.getMaxV();
	return a;
	}

	private double evalG(MSeq<Chromosome<BitGene>> c){
		Genotype<BitGene> g = new Genotype<BitGene>(c.toISeq());
		//g.newInstance(c.toISeq());
		return RealFunction5Rastrigin.eval(g);
	}
