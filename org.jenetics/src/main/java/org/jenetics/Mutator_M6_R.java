...
import org.jenetics.example.Rastrigin;
	...
	extends AbstractAlterer<DoubleGene, C>
{
	 ...
	 */
	public Mutator_M6_R(final double probability) {
		super(probability);
	}
	...
	private int mutate(final MSeq<Chromosome<DoubleGene>> c, final int i, final double p) {
		...
		genesM.set(0, DoubleGene.of(Math.max(val-change, Rastrigin.getMinV()), Rastrigin.getMinV(), Rastrigin.getMaxV()));
		genesW.set(0, DoubleGene.of(Math.min(val+change, Rastrigin.getMaxV()), Rastrigin.getMinV(), Rastrigin.getMaxV()));
...
//				.count();
	...
	/*private DoubleGene upgrade1(DoubleGene x, DoubleGene uy){
		...
		if (Rastrigin.eval2(dx, vW) < Rastrigin.eval2(dx, v)){
		v = vW;
		}
		if (Rastrigin.eval2(dx, vM) < Rastrigin.eval2(dx, v)){
			v = vM;
		}
		//System.out.println("v2:"+v);

		return DoubleGene.of(v,Rastrigin.getMinV(),Rastrigin.getMaxV());
	}*/

	private double testD(double a){
	if(a < Rastrigin.getMinV()) return Rastrigin.getMinV();
	if(a > Rastrigin.getMaxV()) return Rastrigin.getMaxV();
	return a;
	}
